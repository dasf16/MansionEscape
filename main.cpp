...
void mouse_button_callback(GLFWwindow* window, int button, int action, int mods)
{
	if (button == GLFW_MOUSE_BUTTON_LEFT && action == GLFW_PRESS && xPos > 561 && xPos < 905 && yPos < 587 && yPos > 516)
	{
		...
		if (SoundEngine != nullptr)
		{
			...
			fireplaceAudio->setMinDistance(3.0f);
...
int WINAPI wWinMain( HINSTANCE hInstance, HINSTANCE hPrevInstance, LPWSTR lpCmdLine, int nCmdShow )
{
	...
	walkingAudio = SoundEngine->play2D("Audio/walk.mp3",GL_FALSE, GL_TRUE);
	//walkingAudio->setVolume(0.2f);
	pianoSolved = SoundEngine->play2D("Audio/PianoSolved.mp3", GL_FALSE, GL_TRUE);
	clockChimes = SoundEngine->play2D("Audio/clock4ChimesFade.mp3", GL_FALSE, GL_TRUE);
	backgroundMusic = SoundEngine->play2D("Audio/amnesia.mp3", GL_TRUE, GL_TRUE);
	...
	while (!glfwWindowShouldClose(gWindow)) ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	{
		...
		{
			walkingAudio->setIsPaused(false);
			if (walkingAudio->isFinished() && firstStep == 1)
			{
				walkingAudio = SoundEngine->play2D("Audio/walk.mp3", GL_FALSE, GL_TRUE);
				//walkingAudio->setVolume(0.8f);
				firstStep = 0;
				secondStep = 1;
			}
			if (walkingAudio->isFinished() && secondStep == 1)
			{
				walkingAudio = SoundEngine->play2D("Audio/walk2.mp3", GL_FALSE, GL_TRUE);
				//walkingAudio->setVolume(0.8f);
				firstStep = 1;
				secondStep = 0;
			}
		...
		//-----After the clock's puzzle solved sound is finished, background music will resume-----//
		if (clockChimes->isFinished())
		{
			backgroundMusic->setIsPaused(false);
		}
		//-----Same for the piano's puzzle solved sound-----//
		if (pianoSolved->isFinished())
		{
			backgroundMusic->setIsPaused(false);
		}
		...
		if (gameState == 4)
		{
			...
			if (pianoDone == true || GLFW_PRESS == glfwGetKey(gWindow, GLFW_KEY_R))
			{
				if (pianoDone == true)
				{
					//pianoSolved = SoundEngine->play2D("Audio/PianoSolved.mp3", GL_FALSE);
					pianoSolved->setIsPaused(false);
		...
		if (gameState == 1)
		{
								...
								if (clockCode[1] == 3)
								{
									if (clockCode[2] == 0)
									{
										...
										if (clockCode[3] == 4)
										{
											//du klarade det
											backgroundMusic->setIsPaused(true);
											//clockChimes = SoundEngine->play2D("Audio/clock4ChimesFade.mp3", GL_FALSE);
											clockChimes->setIsPaused(false);
											clockAudio->stop();
											clockMenyON = false;
											inventoryON = false;
			...
			if (renderFire == true)
			{
				fireParticle1.update(glm::vec3(77.859, 1.0, 11.713), dt, 8, 5, 100, 50, glm::vec3(0.025, 0.06, 0.025), glm::vec3(-0.025, 0.005, -0.025), glm::vec3(0, 0, 0),
				...
				smokeParticle1.update(glm::vec3(77.859, 1.0, 11.713), dt, 8, 5, 100, 50, glm::vec3(0.025, 0.1, 0.025), glm::vec3(-0.025, 0.005, -0.025), glm::vec3(0, 0, 0),
				...
				sparkParticle1.update(glm::vec3(77.859, 1.0, 11.713), dt, 10, 2, 100, 50, glm::vec3(0.05, 0.05, 0.05), glm::vec3(-0.05, 0.01, -0.05), glm::vec3(0, 0, 0),
			...
			{
				candle1.update(glm::vec3(39.631f, 15.032f, 31.566f), dt, 8, 5, 20, 50, glm::vec3(0.0025, 0.016, 0.0025), glm::vec3(-0.0025, 0.0015, -0.0025), glm::vec3(0, 0, 0),
			...
			{
				candle2.update(glm::vec3(45.795f, 15.032f, 27.064f), dt, 8, 5, 20, 50, glm::vec3(0.0025, 0.016, 0.0025), glm::vec3(-0.0025, 0.0015, -0.0025), glm::vec3(0, 0, 0),
					glm::vec4(0, 1, 0, 0.5), glm::vec4(0, 0.5, 0.0, 0.5), 0.35f, 0.01f, 0.0f, 0.0f);
			...
			{
				candle3.update(glm::vec3(38.102f, 15.032f, 27.032f), dt, 8, 5, 20, 50, glm::vec3(0.0025, 0.016, 0.0025), glm::vec3(-0.0025, 0.0015, -0.0025), glm::vec3(0, 0, 0),
					glm::vec4(0, 1, 0, 0.5), glm::vec4(0, 0.5, 0.0, 0.5), 0.35f, 0.01f, 0.0f, 0.0f);
			...
			{
				candle4.update(glm::vec3(44.35f, 15.032f, 31.577f), dt, 8, 5, 20, 50, glm::vec3(0.0025, 0.016, 0.0025), glm::vec3(-0.0025, 0.0015, -0.0025), glm::vec3(0, 0, 0),
					glm::vec4(0, 1, 0, 0.5), glm::vec4(0, 0.5, 0.0, 0.5), 0.35f, 0.01f, 0.0f, 0.0f);
			...
			{
				candle5.update(glm::vec3(41.941f, 15.032f, 24.272f), dt, 8, 5, 20, 50, glm::vec3(0.0025, 0.016, 0.0025), glm::vec3(-0.0025, 0.0015, -0.0025), glm::vec3(0, 0, 0),
					glm::vec4(0, 1, 0, 0.5), glm::vec4(0, 0.5, 0.0, 0.5), 0.35f, 0.01f, 0.0f, 0.0f);
			...
			{
				OutputDebugStringA("nice key\n");
				SoundEngine->play2D("Audio\D.wav",GL_FALSE);	//Scary sound here / demonPuzzle // ljudet spelas inte upp här
				renderGoldkey = true;
			}
